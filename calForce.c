// ##################################################
// #   calForce.c - finally revised on Apr 2022     #
...
void CalcRepulsiveForcesSubroutine(int *ind, int *nlPnt, double rPnt[][NDIM], 
  ...
  int chkErr;
  double f, len, dist2, fi[NDIM], dr[NDIM], ratio[4];
  double stf;
...
void CalcSpringForcesSubroutine(int ind1, int ind2, int loc, 
		int *chkBondForce) {
  double dr[NDIM], fi[NDIM], f, f2, len, ratio, rPos[NDIM], drAxis[NDIM];
...
void CalcMotorBackboneForces(void) {
  int n, k, ind, locInd, locInd2, *pI, *chkAbpBF, chkErr;
