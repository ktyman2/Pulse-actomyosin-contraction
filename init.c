// ##################################################
// #   init.c - finally revised on Apr 2022         #
...
void InitOutput(void) { 
  ...
  if (nAct > 0) {
	if (actAss.gTgl != 0 || actDis.gTgl != 0) 
	{ Printf0("nActM/nActA/"); }
	Printf0(" nAct ");
	...
	if (actDis.gTgl != 0) { Printf0("cAcDs "); }
  }
  if (nAbp - nMot > 0) { 
...
void AllocArrays (void) {
  ...
  if (rho.tgl != 0) {
	MALLOC(act.rho,int,nActC);
	MALLOC(abp.rho,int,nAbpC);
	MALLOC(abp.kind,int,nAbpC);
	MALLOC(rho.reg,int,V3PROD(rho.nReg));
  }
  ...
  if (actDis.gTgl != 0 || actNuc.gTgl != 0 || actAss.gTgl != 0) {
...
void AssignArrayValues(void) {
  ...
  if (rho.tgl != 0) {
	memset(act.rho, -1, sizeof(int) * nActC);
	memset(abp.rho, -1, sizeof(int) * nAbpC);
	memset(abp.kind, -1, sizeof(int) * nAbpC);
	memset(rho.reg, -1, sizeof(int) * V3PROD(rho.nReg));
  }
  /*--------------------------------------------------------------------------*/
  ...
  if (acpUnb.gTgl != 0 || acpReb.gTgl != 0 || motUnb.gTgl != 0 
		...
		|| motInaUnb.gTgl != 0 || actDis.gTgl != 0) {
...
void AssignInitValues(void) {
  int n, k, sizeRankArr, ind, adjCpu;
  int oft[NDIM], iAdjCell[NDIM], *chkCpu;
  double ratio, minDim, dimDomC[NDIM];
  ...
  abpF.drag[2].n = (motSA.gTgl != 0) ? 1. : 
		CYL_DRAG(DIA_CYL_MOT, L_MOT_ARM) / actF.dragR;
  for (k = 0; k < NK_ABP; k++) {
  ...
  /*--------- Dynamic behaviors of actin, ACP, motor, and boundaries ---------*/
...
/////////////////////////////////////////
  if (rho.prdR < 0) {
	rho.tgl = 0;
	memset(&rho.prd, 1, sizeof(int));
  }
  else {
	rho.tgl = 1;
	rho.prd = T_SEC2TS(rho.prdR);
  }
  if (rho.tgl != 0) {
	rho.nTotReg = V3PROD(rho.nReg);
	V3DIV(rho.widReg, dimDom, rho.nReg);
	rho.dlyMot = T_SEC2TS(rho.dlyMotR);
	rho.durMot = T_SEC2TS(rho.durMotR);
	rho.dlyAct = T_SEC2TS(rho.dlyActR);
	rho.durAct = T_SEC2TS(rho.durActR);
	if (rho.dlyMot + rho.durMot > rho.prd) {
		Printf0("Error: duration of rho activation should be equal to or "
				"longer than the sum of delay and duration of motor "
				"activation.");
		exit(-1);
	}
	if (rho.dlyAct + rho.durAct > rho.prd) {
		Printf0("Error: duration of rho activation should be equal to or "
				"longer than the sum of delay and duration of altered "
	...
	rho.nFor = (int)(rho.RFor * nActGoal * nActPerSeg);
	rho.frac = 0.;
  }
...
void InitRecFiles(void) {
  int n, cntFileL;
  char fileL[][17] = {"AbpLongFor", "AbpTurnover", "AbpUnb", "AbpDyn", 
		"AbpInstLenS", "AbpInstLenD", "AccuConf", 
		"AllChainList", "AllInstFor", "AllInstOri", "ActInstLenS", 
		"ActInstLenD", "ActFilaLen",  
